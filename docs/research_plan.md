...
Phase 0–2: đã triển khai và kiểm thử. Phase 3/5/6: full user-run 1m/5m đã xác minh bằng checkpoint/artifact hashes. Phase 4A–4B: thư viện fit đã dùng trong Phase 5. Phase 7A: full user-run bốn trial 30/120 đã kiểm tra checkpoint và hai báo cáo; winner chưa khóa. Phase 7B: code, `.bat`, tests và bounded real-data smoke đã có; full user-run/báo cáo chưa xác minh. Phase 8–10: chưa triển khai. Người dùng xác nhận timestamp là đầu nến; timezone nguồn và rollover vẫn chưa xác minh, như ghi trong [data contract](data_contract.md).
