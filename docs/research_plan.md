...
| 9. Chẩn đoán \(z_t\), so sánh mở rộng và mô phỏng | Xem mục "Phase 9 đề xuất" | Mọi kết quả ghi rõ là mang tính khám phá | ⏳ Đề xuất, **chưa duyệt** |
...
## Phase 9 đề xuất: chẩn đoán \(z_t\), so sánh mở rộng và mô phỏng (chưa duyệt)

Phần này là đề xuất, **chưa phải cho phép triển khai**. Mỗi mục cần thảo luận và duyệt riêng. Vì tập kiểm thử cuối 2025–2026 đã dùng một lần, mọi kết quả Phase 9 đều **mang tính khám phá**. DEC-005 và PHASE8-V1 giữ nguyên.

- **9A — Chẩn đoán \(z_t\) trên tập xác thực 2022–2024**, dùng tệp kết quả đã có: hàm tự tương quan (ACF) của \(z\) và \(z^2\); \(|z|\) theo giờ trong phiên (mở cửa, trước nghỉ trưa, ATC); PIT và vượt khoảng theo giờ và theo trạng thái biến động. Mục tiêu: xác định phụ thuộc còn lại đến từ σ (ví dụ tính mùa vụ trong phiên) hay từ dạng \(F\). Khối lượng nhỏ, AI có thể chạy.
- **9B — So sánh 9 họ với \(H=30\) trên tập xác thực:** lấp chỗ trống vì Phase 7A chỉ thử phân phối thực nghiệm và hỗn hợp chuẩn. Dùng lại thư viện Phase 4 và chương trình Phase 5.
...
- **9D — Nếu 9A cho thấy σ là nguyên nhân chính:** một thiết kế mới với σ có điều chỉnh mùa vụ trong phiên, rồi so lại các \(F\). Chỉ khẳng định được trên dữ liệu sau 2026-07-17.
...
Từ 2026-09-25 (DEC-006), phạm vi chỉ còn câu hỏi về phân phối của \(z_t\). Phase 9 (9A–9F, mang tính khám phá) đang ở trạng thái đề xuất, chưa duyệt; Phase 10 chưa bắt đầu. Báo cáo tổng hợp kết quả chính thức: [Bao_cao/bao_cao_ket_qua.tex](../Bao_cao/bao_cao_ket_qua.tex). Người dùng đã xác nhận mốc thời gian là đầu nến; múi giờ nguồn và quy tắc đáo hạn hợp đồng vẫn chưa xác minh (xem [data contract](data_contract.md)).
